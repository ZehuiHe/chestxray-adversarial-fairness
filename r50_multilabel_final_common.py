...
def dynamic_grl_lambda(progress: float, lambda_max: float, gamma: float) -> float:
    """DANN-style smooth schedule from 0 toward lambda_max."""
    progress = float(np.clip(progress, 0.0, 1.0))
    return float(lambda_max * (2.0 / (1.0 + np.exp(-gamma * progress)) - 1.0))
...
def train_dynamic_adv_one_epoch(
    ...
    for batch_index, (images, targets, sexes, _, _) in enumerate(loader, start=1):
        ...
        lambda_current = dynamic_grl_lambda(progress, lambda_max, gamma)
